...
public:
    ...
    void bfs_worker(int start, int end, vector<int>& next_frontier, atomic<int>& next_index) {
    ...
    bool parallelBFS(int s, int t) {
        ...
        int partition_size = V / NUM_THREADS;
        ...
        while (!frontier.empty()) {
            ...
            for (int i = 0; i < NUM_THREADS; i++) {
                ...
                int end = min((i + 1) * partition_size, V);
                threads.emplace_back(&Dinic::bfs_worker, this, start, end, ref(next_frontier), ref(next_index));
            }
